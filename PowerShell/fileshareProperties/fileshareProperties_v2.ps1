...
# headings
"""Path"",""Type"",""Size ($unit)"",""Primary Security Group"",""Last Modified""" | Out-File -FilePath $outfileName
...
function listdir($thisView, $dirPath, $thisDepth){
    ...
    $share = $view.smbMountPaths.trim('{}')
    foreach($entry in $entries.entries){
        $path = $entry.fullPath.replace('/', '\')
        $ACL = (Get-Item "$share$path" | get-acl).Access | Where-Object inheritanceflags -eq ‘none’
        $folderSize = Get-ChildItem "$share$path" | Measure-Object -Property Length -sum
        if($entry.type -eq 'kDirectory'){    
            Write-Host "$($entry.fullPath) ($(usecsToDate $entry.fstatInfo.mtimeUsecs))"
            """$($entry.fullPath)"",""$($entry.type.subString(1))"",""$(toUnits $folderSize)"",""$($ACL.IdentityReference)"",""$(usecsToDate $entry.fstatInfo.mtimeUsecs)""" | Out-File -FilePath $outfileName -Append
        }
        if($entry.type -eq 'kFile' -and $showFiles) {
            Write-Host "$($entry.fullPath) ($(usecsToDate $entry.fstatInfo.mtimeUsecs))"
            """$($entry.fullPath)"",""$($entry.type.subString(1))"",""$(toUnits $entry.fstatInfo.size)"",""$($ACL.IdentityReference)"",""$(usecsToDate $entry.fstatInfo.mtimeUsecs)""" | Out-File -FilePath $outfileName -Append
        }
        if($entry.type -eq 'kDirectory'){
