...
# ./pullDMaaSAuditLogs.ps1 -apiKey #### -regionAll
...
param (
    ...
    [Parameter()][array]$regionId,  # DMaaS region ID(s) (comma separated)
    [Parameter()][switch]$regionAll, # switch to indicate that ALL DMaaS Regions will be pulled for Audit Report (recommended)
...
$outfileName = "$PSScriptRoot\log-pullDMaaSAuditLogs-$dateString.txt"
...
if(!$tenantId){
    write-host "No DMaaS Tenant ID found!`n" -ForegroundColor Yellow
    write-output "$dateTime    WARN    No DMaaS Tenant ID found!`n" | out-file -filepath $outfileName -Append
}
else{
...
if($regionAll -eq $true){
    ...
    # if($invRegion){
    #     write-host "`nThe DMaaS Region Id $invRegion is NOT asssociated with the specified Tenant ID!`n" -ForegroundColor Yellow 
    #     write-output "`n$dateTime    WARN    The DMaaS Region Id $invRegion is NOT asssociated with the specified Tenant ID!`n" | Out-File -FilePath $outfileName -Append 
    # }
    
    $regionIds = ($verRegion -join ",")

    if($regionIds){
        Write-Host "DMaaS Region ID Verified: $regionIds`n" -ForegroundColor Green
        write-output "$dateTime    INFO    DMaaS Region ID Verified: $regionIds`n" | Out-File -FilePath $outfileName -Append 
    }
    else{
        write-host "The DMaaS Region Id $regionIds is NOT asssociated with the specified Tenant ID!`n" -ForegroundColor Yellow 
        write-output "$dateTime    WARN    The DMaaS Region Id $regionIds is NOT asssociated with the specified Tenant ID!`n" | Out-File -FilePath $outfileName -Append 
    }  
}
...
# pulling DMaaS Audit Logs and saving to delimited csv format
if($days -gt 1){
    Write-Host "`nPulling DMaaS Audit Logs from $days days ago...`n" 
    write-output "`n$dateTime    INFO    Pulling DMaaS Audit Logs from $days days ago...`n" | Out-File -FilePath $outfileName -Append 
}
else{
    Write-Host "`nPulling DMaaS Audit Logs from $days day ago...`n" 
    write-output "`n$dateTime    INFO    Pulling DMaaS Audit Logs from $days day ago...`n" | Out-File -FilePath $outfileName -Append 
}
...
if($actions) {
    ...
    # write-host "https://helios.cohesity.com/v2/mcm/audit-logs?startTimeUsecs=$starttimeusecs&serviceContext=Dmaas&regionIds=$regionIds&endTimeUsecs=$endtimeusecs&count=1000&actions=$actionsJn"
    ...
    if($response){
        ...
        Write-Output "$jsonLogs" | Set-Content -path $auditLogs\DMaaS-Audit-Logs_$dateString.json

        # write-host $jsonLogs 

        write-host "DMaaS Audit Logs SUCCESSFULLY saved to: $auditLogs\DMaaS-Audit-Logs_$dateString.json`n" -ForegroundColor Green
        write-output "$dateTime    INFO    DMaaS Audit Logs SUCCESSFULLY saved to: $auditLogs\DMaaS-Audit-Logs_$dateString.json`n" | Out-File -FilePath $outfileName -Append
    }
    else{
    ...
    write-host "Audit Pull FAILED! The appropriate DMaaS Parametes could not be called.`n" -ForegroundColor Red
    write-output "$dateTime    FAIL    Audit Pull FAILED! The appropriate DMaaS Parametes could not be called.`n" | Out-File -FilePath $outfileName -Append
}
