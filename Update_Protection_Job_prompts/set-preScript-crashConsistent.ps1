...
$JobNameFile = Read-Host -Prompt 'Please input the path and filename of the Protection Job Names text file (ex: C:\Documents\JobNames.txt)'

#connect to Cohesity PowerShell API
Connect-CohesityCluster -Credential (Get-Credential $Credentials) -Server $server -Port 46258

#references the content of the Cohesity Protection Job Names text file.
$ProtectionJobNames = Get-Content -Path "$JobNameFile"

#iterates through each Cohesity Protection Job Name
foreach ($JobName in $ProtectionJobNames)
{
    ...
    $backupObj.preBackupScript.incrementalBackupScript.scriptPath = "CoBMR.bat"

    #updates the Protection Job Crash Consistency
    $backupObj.quiesce = "True"
