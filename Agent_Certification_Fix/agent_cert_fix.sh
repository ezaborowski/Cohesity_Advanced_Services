...
function write_log {
    ...
    printf '\n' | tee -a "$log_file"
    printf '\n' | tee -a "$log_file"
    echo "*** Once the agent_installer_files.json has been properly updated, please browse to the Cohesity UI > Data Protection > Sources > and use the Upgrade Agents to install the new Agents with updated Certificates. ***" | tee -a "$log_file"
    printf '\n' | tee -a "$log_file"
    printf '\n' | tee -a "$log_file"
